...
fn parse_table(sources: impl IntoIterator<Item = ConfigSource>) -> Map<String, Value> {
...
fn config_item_ts(config_item: ConfigItem) -> proc_macro2::TokenStream {
    ...
    let table = VariantRepresentation::represent_table(Self::parse_table(sources));
    let select_impls = proc_macro2::TokenStream::from_iter(Self::iter_select_impls(&name, &table));
    let type_ts = VariantRepresentation::type_ts_container(&table);
    let value_ts = VariantRepresentation::value_ts_container(&table);
    quote::quote! {
        #(#attrs)*
...
pub(crate) fn config_items_ts(config_items: ConfigItems) -> proc_macro2::TokenStream {
    proc_macro2::TokenStream::from_iter(config_items.items.into_iter().map(Self::config_item_ts))
}
